import{P as y}from"./pageTypes-s5bfgjb6.js";import{b as $}from"./index-jMcgm5iu.js";import{s as v,r as w}from"./pageUi-D8KbPjWU.js";const k={type:y.TABLE_COMPLETE,initState:()=>({values:{},submitted:!1,isCorrect:null,feedback:""}),render:({page:r,pageState:e})=>{const t=Array.isArray(r?.columns)?r.columns:[],n=Array.isArray(r?.rows)?r.rows:[],a=e?.values??{},s=t.map(c=>`<th>${c}</th>`).join(""),u=n.map((c,l)=>`<tr>${c.map((o,f)=>{if(o==null){const b=`${l},${f}`,m=a[b]??"";return`<td><input data-row="${l}" data-col="${f}" type="text" value="${m}"></td>`}return`<td>${o}</td>`}).join("")}</tr>`).join(""),d=w(e);return`
